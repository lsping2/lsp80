...
if ( $_SERVER['REMOTE_ADDR'] == "121.129.204.34"){
	 //print_r($contents);
	// $xml = simplexml_load_string ($contents);
	//echo "==>".$xml->response[0]->nickname;
	//exit;
}
		$xml = simplexml_load_string ($contents);
	...
	$content['email'] = $xml->response[0]->email;

  // $content['name'] = (string) $xmlr->name->{0}; //추후 네이버에서 지원한다고 함
   $content['name'] = $xml->response[0]->name;

  // $content['nickname'] = (string) $xmlr->nickname->{0};
  $content['nickname'] = $xml->response[0]->nickname;
