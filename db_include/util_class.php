...
function my_smail( $send_name, $from_email, $returned_email, $to_email, $subject, $comment )
{
	...
	*/
	...
	$boundary = '=_'.md5(uniqid(time())); 
        ...
        $from = "From: =?ks_c_5601-1987?B?$from?= <lsp80@coowoo.com>";
...
function CutString( $strString, $iLength )
{
    ...
    if ($start > 0) {
        for($i=0;$i<=$start;$i++) {
	        if (ord(substr($strString,$i,1))>127) {
	            $k++;
            }
            if (!($k%2)) {
	            $start++;
	...
	if ( $iLength < $str_len ) {
	    return substr($strString,$start,$iLength)."..";
	} else {
		return $strString;
	}
}

function get_name ($branch_cd , $jisa_cd){
	if ( $_SESSION[USER_LEVEL] > "2" )
	{
		$db = new MYSQL();
		if ( $_SESSION[USER_LEVEL] == "99" ){
			$query = "select	branch_nm  from	memb003 where	branch_cd	= '$branch_cd' ";						
			$db->query( $query );
			$db->fetch();
			$branch_nm = $db->result('branch_nm');
		}else{
			$query = "select	memb_nm   from	memb001 where	jisa_cd =  '$jisa_cd' and user_level='2' ";						
			$db->query( $query );
			$db->fetch();
			$branch_nm = $db->result('memb_nm');
		}
		return "(".$branch_nm.")";
		$db-> close();
	}
...
function get_name_sub ($branch_cd , $jisa_cd){
			$db = new MYSQL();
			$query = "select	branch_nm as memb_nm   from	memb003 where	jisa_cd =  '$jisa_cd' and branch_cd= '$branch_cd' ";						
			$db->query( $query );
			$db->fetch();
			$branch_nm = $db->result('memb_nm');
