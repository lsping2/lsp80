...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_atc">
        ...
        <tbody>
        <tr>
            <th scope="row"><label for="val1">항목1<strong class="sound_only">필수</strong></label></th>
            <td>
					<?
						$val1 = explode(",",$view['val1']);
						$val2 = explode(",",$view['val2']);
					?>
					<? for($loop=0; $loop < $view['val_ct']; $loop++) :?>
								<? if ( $val1[$loop] != "-") :?>
									<?=$val1[$loop]?> - <?=$val2[$loop]?><br>
								<? endif ?>
					<? endfor ?>
			</td>
        </tr>
		...
		<tr>
            <th scope="row"><label for="val1">항목2<strong class="sound_only">필수</strong></label></th>
            <td>
					<?
						$sel1 = explode(",",$view['sel1']);
						$sel2 = explode(",",$view['sel2']);
						$sel3 = explode(",",$view['sel3']);
					?>
					<? for($loop=0; $loop < $view['sel_ct']; $loop++) :?>
							<? if ( $sel2[$loop] != "-") :?>
								<?=$sel1[$loop]?> - <?=$sel2[$loop]?>- <?=$sel3[$loop]?><br>
							<? endif ?>
					<? endfor ?>
			</td>
        </tr>
