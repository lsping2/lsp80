...
$total_count = 0;
 ...
 for ($i=0; $i<count($arr_bo_table); $i++) {
	 $tmp_write_table   = "g5_write_" . $arr_bo_table[$i];
...
$from_record = ($page - 1) * $rows; // 시작 열을 구함
 ...
 for ($i=0; $i<count($arr_bo_table); $i++) {
	 ...
	 $sql = " select bo_subject from {$g5['board_table']} where bo_table = '{$arr_bo_table[$i]}' ";
	 $row = sql_fetch($sql);
	...
	for ($k=0; $row=sql_fetch_array($result); $k++) {
			echo "<a href='/bbs/board.php?bo_table=".$arr_bo_table[$i]."&wr_id=".$row['wr_parent']."'>".$bo_subject[$i]."==".$row[wr_subject].'</a>';
			echo "<br>";
	}
 }
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class="" align="center">
				<? if($list[$i]['sub_date'] != '0000-00-00 00:00:00') :?>
						 <a href="<?php echo $list[$i]['href'] ?>">등록완료</a>
				<? endif ?>
			</td>
    ...
    <fieldset id="bo_sch">
        ...
        <select name="sfl" id="sfl">
			...
			<option value="wr_1,1"<?php echo get_selected($sfl, 'wr_1,1'); ?>>담당ID</option>
